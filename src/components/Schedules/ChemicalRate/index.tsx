...
export const ChemicalRateSchedule: FC<TProps> = ({ currentValue, scheduleName, valueLabel }) => {
  const options = {
    responsive: true,
      ...
      tooltip: {
        ...
        callbacks: {
          ...
          title(tooltipItems: any) {
            return `${tooltipItems[0].formattedValue}${valueLabel}`
          },
      ...
      y: {
        ...
        ticks: {
          ...
          callback: (value: number) => {
            return value % 50 === 0 ? `${value}${valueLabel}` : '';
          },
        },
        grid: {
          color: (context: ScriptableContext<'linear'>) => {
            const index = Number(context.tick.value);
            return index % 50 === 0 ? '#E5EFFF' : 'rgba(117,206,196,0)';
          }
        }
      },
      x: {
